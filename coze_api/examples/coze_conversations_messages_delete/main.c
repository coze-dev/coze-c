...
int main() {
    ...
    const coze_conversations_messages_delete_request_t req = {
        .api_token = api_token,
        .conversation_id = conversation_id,
        .message_id = message_id,
    };
    coze_conversations_messages_delete_response_t resp = {0};
    const coze_error_t err = coze_conversations_messages_delete(&req, &resp);
    if (err != COZE_OK) {
        fprintf(stderr, "Error deleting conversation message: %d, code: %d, msg: %s\n", err, resp.code, resp.msg);
        return 1;
    }
    printf("log_id: %s\n", resp.response.logid);
    printf("message.id: %s\n", resp.data.id);
    ...
    coze_free_conversations_messages_delete_response(&resp);
    return 0;
}
